...
| command | median |
| --- | --- |
| `bunx -y ccstatusline-jb@latest` | 633 ms |
| `bunx -y ccstatusline-jb@2.2.27` | 202 ms |
| `bunx -y ccstatusline-jb` | 207 ms |

Dropping `@latest` is worth about 430 ms per repaint there. npm does not behave this way: `npx -y
ccstatusline-jb@latest` and `npx -y ccstatusline-jb@2.2.27` measured 1082 ms and 1135 ms, so pinning
buys nothing under `npx`. A **Pinned global install**, which writes `"command": "ccstatusline"`,
avoids the resolution entirely on both.
